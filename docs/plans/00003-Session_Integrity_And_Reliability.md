...
| 2026-09-21T13:17:39Z | STEP-06 | The step asked for a test that standard input reaches the child. Reading standard input in a test can block when it is a terminal, which `AGENTS.md` forbids, so the test compares what `/proc/self/fd/0` points at in the child and in the test process. It is exact, cannot block, and is `#[cfg(target_os = "linux")]`; on other platforms inheritance rests on `Stdio::inherit()` alone | AC-12 is met on Linux, which is where CI runs; macOS has no equivalent check | User, at hand-off |
